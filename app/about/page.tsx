...
function StatCard({ value, suffix, prefix, label, sub, accent }: {
  ...
  return (
    <div className="ro-stat-card" style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", border: "1px solid #e8e8e8", boxShadow: "0 2px 12px rgba(0,0,0,.06)", position: "relative", overflow: "hidden", boxSizing: "border-box" as const }}>
...
// ── Team Card ────────────────────────────────────────────────────────────────
function TeamCard({ initial, name, role, color }: { initial: string; name: string; role: string; color: string }) {
  return (
    <div className="ro-team-card" style={{ background: "#fff", borderRadius: 14, padding: "24px 20px", textAlign: "center", border: "1px solid #e8e8e8", boxShadow: "0 2px 8px rgba(0,0,0,.05)", boxSizing: "border-box" as const }}>
    ...
    <div style={{ minHeight: "100vh", background: "#f5f5f5", fontFamily: "'Helvetica Neue',Arial,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "72px 20px" }}>
        <div className="ro-about-grid">
          ...
          {/* Right — quotes/facts */}
          <div className="ro-right-col" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "#1e1e1e", padding: "72px 20px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ color: "rgba(255,255,255,.7)", fontSize: 15, lineHeight: 1.8, marginBottom: 48 }}>
            When a trader buys a YES contract on a student passing their class, something shifts. The community has a stake in the outcome. The market price becomes a real-time signal of confidence.
          </p>
          <div className="ro-mission-grid">
            {[
              ...
              <div key={title} className="ro-mission-card" style={{ background: "rgba(255,255,255,.05)", borderRadius: 14, padding: "24px 20px", border: "1px solid rgba(255,255,255,.08)", textAlign: "left", boxSizing: "border-box" as const }}>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "72px 20px" }}>
        ...
        <TimelineItem year="HR 1" title="The Idea" body="Four UNLV students, frustrated by Las Vegas's education statistics, ask a simple question: could prediction markets change how students relate to their own academic futures?" />
        <TimelineItem year="HR 4" title="Research & Validation" body="The team dives into CCSD data. They find staggering gaps: 30% math proficiency, 33% chronic absenteeism, an 18.5% dropout rate." />
        <TimelineItem year="HR 8" title="Building the MVP" body="Armed with a Supabase backend, a Next.js frontend, and an AMM-based pricing engine, the team builds the first version of Rebel Odds. Mock contracts, paper tokens, real market mechanics. The hackathon clock starts ticking." />
        <TimelineItem year="HR 24" title="Launch" body="Rebel Odds goes live as a paper-trading platform. No real money. Real accountability." last />
      </div>
      ...
      <div style={{ background: "#fff", borderTop: "1px solid #e8e8e8", padding: "72px 20px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="ro-team-grid">
            <TeamCard initial="D" name="Dylan Aguilar" role="Frontend" color="#E31837" />
            <TeamCard initial="A" name="Andre Lee" role="Database" color="#1e1e1e" />
            <TeamCard initial="G" name="Gongsak Kittiyan" role="Frontend" color="#2d8a4e" />
            <TeamCard initial="V" name="Victor Sarmiento" role="Backend" color="#A03123" />
          </div>
        </div>
      </div>
      ...
      <style>{`
        /* ─── Stats: always 3 per row, last 2 centered ─── */
        .ro-stats-grid {
          display: flex;
          flex-wrap: wrap;
          gap: 16px;
        ...
        /* ─── Origin story: 2-col desktop, stacked mobile ─── */
        .ro-about-grid {
          display: grid;
        ...
        @media (max-width: 860px) {
          ...
          .ro-left-col { order: 1; }
          .ro-right-col { order: 2; }
        }

        /* ─── Mission: all 4 across → 2×2 → all stacked ─── */
        .ro-mission-grid {
          display: flex;
          flex-wrap: wrap;
          gap: 20px;
        ...
        /* ─── Team: all 4 across → 2×2 → all stacked ─── */
        .ro-team-grid {
          display: flex;
          flex-wrap: wrap;
          gap: 16px;
